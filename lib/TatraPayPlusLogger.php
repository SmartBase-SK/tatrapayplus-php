...
class TatraPayPlusLogger
{
    ...
    private function _mask_value($value, int $max_chars = 5): string
    {
        if (!is_string($value) || strlen($value) < $max_chars * 2) {
            return str_repeat("*", strlen($value));
        }
        return substr($value, 0, $max_chars) .
            str_repeat("*", strlen($value) - $max_chars * 2) .
            substr($value, -$max_chars);
    }

    private function _mask_header(array $header): false|string
    {
    ...
    private function _mask_body(mixed $body): mixed
    {
        if (empty($body)) {
            return "No body";
        }
        ...
        if (is_string($body)) {
            ...
            foreach ($pairs as $key => $value) {
                if (in_array($key, $this->mask_body_fields, true)) {
                    $masked_pairs[] =
                        $key . "=" . $this->_mask_value((string) $value);
                } else {
                    print $value;
    ...
    public function log(HttpResponse $response, $additional_data): void
    {
        ...
        $this->writeLine(
            sprintf(
                "INFO [%s] [INFO] Response (status code: %s):",
                $date,
